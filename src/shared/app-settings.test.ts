...
describe('mergeKunRuntimeSettings', () => {
  ...
  it('allows the Kun API protocol to switch back from Responses', () => {
    ...
    expect(current.apiType).toBe('responses')
    expect(next.apiType).toBe('chat_completions')
  })
...
describe('write inline completion runtime config', () => {
  ...
  it('inherits the provider model when the Kun model is still the built-in default', () => {
    const state = settings()
    ...
    expect(resolveWriteInlineCompletionModel(state)).toBe('gpt-5.5')
  })
