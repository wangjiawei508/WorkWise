...
const LEGACY_COREAGENT_DATA_DIR = '~/.workgpt/coreagent'
...
export function mergeKunRuntimeSettings(
  ...
  return {
    ...current,
    ...(patch ?? {}),
    apiType: normalizeKunRuntimeApiType(patch?.apiType ?? current.apiType),
...
function normalizeKunRuntimeApiType(value: unknown): KunRuntimeApiType {
  if (value === 'responses' || value === 'chat_completions' || value === 'inherit') {
    return value
  }
  return 'inherit'
}
...
function upgradeLegacyKunDefaultDataDir(value: unknown): string {
  ...
  if (
    ...
    normalized.endsWith('/.workgpt/coreagent')
  ) {
