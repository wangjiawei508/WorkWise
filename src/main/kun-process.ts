...
export async function startKunChild(settings: AppSettingsV1): Promise<void> {
  ...
  child = spawn(resolution.command, args, {
    env: {
      ...
      DEEPSEEK_API_KEY: runtime.apiKey || process.env.DEEPSEEK_API_KEY || process.env.OPENAI_API_KEY || '',
      OPENAI_API_KEY: runtime.apiKey || process.env.OPENAI_API_KEY || process.env.DEEPSEEK_API_KEY || ''
    },
