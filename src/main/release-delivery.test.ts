...
describe('R2 release delivery gates', () => {
  ...
  it('keeps website cache repair scoped, backed up, and validated before reload', () => {
    ...
    expect(repair).toContain("grep -Eiq 'server_name[^;]*railwise\\.cn'")
    expect(repair).toContain('nginx_candidate_count')
    expect(repair).toContain("re.search(r'\\bserver_name\\b[^;]*\\brailwise\\.cn\\b'")
    expect(repair).toContain('(?:latest\\.json|latest(?:-mac)?\\.yml)')
    expect(repair).not.toContain("re.search(r'\\\\bserver_name")
