...
const startupTraceEnabled = process.env.WORKGPT_STARTUP_TRACE === '1'
...
function syncLoginItemSettings(settings: AppSettingsV1): void {
  ...
  try {
    ...
    console.warn('[workgpt] failed to update login item settings:', error)
...
async function ensureKunRuntime(settings: AppSettingsV1): Promise<void> {
  ...
  try {
    ...
    console.error('[workgpt] failed to start kun:', e)
    throw e
  }
...
function createWindow(options: { suppressInitialShow?: boolean } = {}): void {
  ...
  mainWindow.webContents.on('preload-error', (_event, preloadPath, error) => {
    const message = error instanceof Error ? error.message : String(error)
    console.error(`[workgpt] failed to load preload ${preloadPath}:`, error)
...
async function restartManagedRuntimeForSettingsChange(
  ...
  try {
    ...
    if (!healthy) {
      console.warn('[workgpt] Kun restart did not become healthy after settings change')
    }
  } catch (e) {
    console.warn('[workgpt] Kun restart failed after settings change:', e)
  }
}
...
async function restartManagedRuntimeForMcpConfigChange(settings: AppSettingsV1): Promise<void> {
  ...
  try {
    ...
    if (!healthy) {
      console.warn('[workgpt] Kun restart did not become healthy after MCP config change')
    }
  } catch (e) {
    console.warn('[workgpt] Kun restart failed after MCP config change:', e)
  }
}
...
app.whenReady().then(async () => {
  ...
  void loadGuiUpdaterModule().catch((error) => {
    console.warn('[workgpt updater] failed to initialize on startup:', error)
  })
...
app.on('window-all-closed', () => {
  void stopManagedRuntimes().catch((error) => {
    console.warn('[workgpt] failed to stop Kun runtime:', error)
  })
