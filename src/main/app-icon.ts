 ...
 * (形如 `'chunks/workgpt-XXXX.png'`)。main bundle 输出在 `out/main/`,所以
 * 运行时 `__dirname = out/main/`,asset 在 `out/main/chunks/workgpt-XXXX.png`。
 *
...
export function createAppIcon(source: string): Electron.NativeImage {
  ...
  try {
    ...
    console.warn(
      '[workgpt] failed to load app icon from',
      absolute,
      '-',
      message
    )
