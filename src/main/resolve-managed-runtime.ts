...
/**
 * Resolve the bundled WorkWise Agent Runtime executable. The audited runtime
 * source remains under the internal `kun/` provenance boundary and is
 ...
 */
export type ManagedRuntimeBinaryResolution =
...
export function resolveManagedRuntimeExecutable(
  appRoot: string,
  userBinaryPath: string
): ManagedRuntimeBinaryResolution {
...
/**
 * Build the full managed runtime argv from resolved binary info.
 * and managed runtime settings. The function is pure: no I/O, no
 * side effects, easy to test.
 */
export function buildManagedRuntimeServeArgs(input: {
  resolution: ManagedRuntimeBinaryResolution
  host: string
  port: number
  dataDir: string
  baseUrl?: string
