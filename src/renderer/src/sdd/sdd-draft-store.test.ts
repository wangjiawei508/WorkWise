...
const SDD_DRAFT_REGISTRY_STORAGE_KEY = 'workgpt.sdd.draft.registry.v1'
...
describe('sdd-draft-store', () => {
  ...
  it('saves the active draft to disk and updates clean state', async () => {
    ...
    window.workgpt.writeWorkspaceFile = writeWorkspaceFile
    const draft = createSddDraft({
      id: '123e4567-e89b-12d3-a456-426614174000',
      workspaceRoot: '/tmp/app',
      now: 1
  ...
  it('keeps the draft dirty when disk save fails', async () => {
    window.workgpt.writeWorkspaceFile = vi.fn().mockResolvedValue({
      ok: false,
