...
import { loadRuntimeDiagnostics } from './load-runtime-diagnostics'

describe('loadRuntimeDiagnostics', () => {
  it('loads runtime info, tool diagnostics, and memory records together', async () => {
    ...
    const loaded = await loadRuntimeDiagnostics(provider, { workspace: '/tmp/project' })
