...
export const SETTINGS_CHANGED_EVENT = 'workgpt:settings-changed'
...
export function useKeyboardShortcutSettings(): KeyboardShortcutsConfigV1 {
  ...
  useEffect(() => {
    ...
    if (typeof window.workgpt?.getSettings === 'function') {
      void window.workgpt.getSettings().then(apply).catch(() => undefined)
    }
