...
describe('write-thread-registry', () => {
  ...
  it('hydrates leaked write assistant threads from configured write workspaces', () => {
    const leaked = {
      ...thread('write-thread', '/Users/zxy/.workgpt/write_workspace'),
      title: WRITE_ASSISTANT_THREAD_TITLE
    }
    const normalCodeThread = {
      ...thread('code-thread', '/Users/zxy/.workgpt/write_workspace'),
    ...
    const registry = hydrateWriteThreadRegistry(
      ...
      ['/Users/zxy/.workgpt/write_workspace'],
      emptyWriteThreadRegistry()
    )
  ...
  it('hydrates legacy tilde write assistant threads under the configured absolute workspace', () => {
    const legacyThread = {
      ...thread('legacy-write-thread', '~/.workgpt/write_workspace'),
      title: WRITE_ASSISTANT_THREAD_TITLE
    }
    ...
    expect(registry.workspaces['/Users/zxy/.workgpt/write_workspace'].threadIds).toEqual([
      'legacy-write-thread'
    ])
    expect(
      activeWriteThreadForWorkspace(
  ...
  it('hydrates Reasonix write-context threads even when the session list reports the default workspace', () => {
    const leaked = {
      ...thread('reasonix-write-thread', '/Users/zxy/.workgpt/default_workspace'),
    ...
    expect(writeWorkspaceForThreadId('reasonix-write-thread', registry)).toBe('/Users/zxy/.workgpt/write_workspace')
    expect(
      activeWriteThreadForWorkspace(
        '/Users/zxy/.workgpt/write_workspace',
        [leaked],
