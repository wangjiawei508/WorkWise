...
export function createWriteFileActions({
  ...
  return {
    ...
    loadDirectory: async (workspaceRoot, path) => {
      ...
      let result: Awaited<ReturnType<typeof window.workgpt.listWorkspaceDirectory>>
      try {
        result = await window.workgpt.listWorkspaceDirectory({ workspaceRoot, path })
      } catch (error) {
        set((state) => ({
    ...
    openFile: async (workspaceRoot, path) => {
      ...
      try {
        if (isWriteImageFilePath(path)) {
          const result = await window.workgpt.readWorkspaceImage({ path, workspaceRoot })
          if (!result.ok) {
        ...
        const result = await window.workgpt.readWorkspaceFile({ path, workspaceRoot })
        if (!result.ok) {
          set({ fileLoading: false, fileError: result.message })
          return
        }
    ...
    createFile: async (workspaceRoot, path, content = '') => {
      let result: Awaited<ReturnType<typeof window.workgpt.createWorkspaceFile>>
      try {
        result = await window.workgpt.createWorkspaceFile({ workspaceRoot, path, content })
      } catch (error) {
    ...
    createDirectory: async (workspaceRoot, path) => {
      let result: Awaited<ReturnType<typeof window.workgpt.createWorkspaceDirectory>>
      try {
        result = await window.workgpt.createWorkspaceDirectory({ workspaceRoot, path })
      } catch (error) {
        set({ fileError: formatActionError(error) })
        return null
      }
    ...
    renameEntry: async (workspaceRoot, path, newName) => {
      cancelExternalSyncAnimation()
      let result: Awaited<ReturnType<typeof window.workgpt.renameWorkspaceEntry>>
      try {
        result = await window.workgpt.renameWorkspaceEntry({ workspaceRoot, path, newName })
      } catch (error) {
        set({ fileError: formatActionError(error) })
        return null
      }
    ...
    deleteEntry: async (workspaceRoot, path) => {
      cancelExternalSyncAnimation()
      let result: Awaited<ReturnType<typeof window.workgpt.deleteWorkspaceEntry>>
      try {
        result = await window.workgpt.deleteWorkspaceEntry({ workspaceRoot, path })
      } catch (error) {
        set({ fileError: formatActionError(error) })
        return false
      }
