...
export const useWriteWorkspaceStore = create<WriteWorkspaceState>((set, get) => ({
  ...
  syncActiveFileFromDisk: async (workspaceRoot, options = {}) => {
    ...
    if (typeof content !== 'string') {
      let result: Awaited<ReturnType<typeof window.workgpt.readWorkspaceFile>>
      try {
        result = await window.workgpt.readWorkspaceFile({
  ...
  syncActiveImageFromDisk: async (workspaceRoot, path) => {
    ...
    try {
      const result = await window.workgpt.readWorkspaceImage({
        path: snapshot.activeFilePath,
        workspaceRoot
      })
      if (!result.ok) {
  ...
  flushSave: async (workspaceRoot) => {
    ...
    try {
      const result = await window.workgpt.writeWorkspaceFile({
