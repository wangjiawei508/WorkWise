...
async function loadTokenEconomySavingsSummary(): Promise<TokenEconomySavingsSummary | null> {
  if (typeof window === 'undefined' || typeof window.workgpt?.runtimeRequest !== 'function') return null
  const response = await window.workgpt.runtimeRequest('/v1/usage?group_by=thread', 'GET')
            ...
            <>
              ...
              <div ref={agentsSectionRef}>
                <SettingsCard title={t('agents')}>
                      ...
                      <div className="grid gap-3 lg:grid-cols-[260px_minmax(0,1fr)]">
                        ...
                        {activeProvider ? (
                          <div className="grid gap-3 rounded-xl border border-ds-border-muted bg-ds-main/35 p-3">
                            ...
                            <label className="grid gap-1.5 text-[12px] font-semibold text-ds-muted">
                              {t('modelProviderApiType')}
                              <select
                                className={selectControlClass}
                                value={activeProvider.apiType}
                                onChange={(e) => updateModelProvider(activeProvider.id, {
                                  apiType: e.target.value === 'responses' ? 'responses' : 'chat_completions'
                                })}
                              >
                                <option value="chat_completions">{t('modelProviderApiTypeChatCompletions')}</option>
                                <option value="responses">{t('modelProviderApiTypeResponses')}</option>
                              </select>
                            </label>
                            <label className="grid gap-1.5 text-[12px] font-semibold text-ds-muted">
                  ...
                  <SettingRow
                    title={t('kunApiType')}
                    description={t('kunApiTypeDesc')}
                    control={
                      <select
                        className={selectControlClass}
                        value={kun.apiType ?? 'inherit'}
                        onChange={(e) => updateKun({
                          apiType: e.target.value === 'responses'
                            ? 'responses'
                      ...
                      >
                        <option value="inherit">{t('modelProviderApiTypeInherit')}</option>
                        <option value="chat_completions">{t('modelProviderApiTypeChatCompletions')}</option>
                        <option value="responses">{t('modelProviderApiTypeResponses')}</option>
                      </select>
                    }
