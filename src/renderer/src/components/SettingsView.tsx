...
type SettingsCategory = 'general' | 'write' | 'agents' | 'shortcuts' | 'claw' | 'help'
...
export function SettingsView(): ReactElement {
  ...
  useEffect(() => {
    let cancelled = false
    if (typeof window.workgpt === 'undefined') {
  ...
  useEffect(() => {
    if (typeof window.workgpt?.getLogPath !== 'function') return
    void window.workgpt.getLogPath().then((p) => setLogPath(p)).catch(() => undefined)
  }, [category])
  ...
  const loadWriteDebugEntries = useCallback(async (): Promise<void> => {
    ...
    try {
      const completionEntries = typeof window.workgpt?.listWriteInlineCompletionDebugEntries === 'function'
        ? await window.workgpt.listWriteInlineCompletionDebugEntries()
        : []
  ...
  useEffect(() => {
    ...
    const refs: Record<Exclude<SettingsRouteSection, 'general' | 'write' | 'claw' | 'shortcuts' | 'help'>, HTMLDivElement | null> = {
  ...
  const loadMcpConfig = async (): Promise<void> => {
    if (typeof window.workgpt?.getDeepseekConfigFile !== 'function') return
    setMcpLoading(true)
    setMcpNotice(null)
    try {
      const config = await window.workgpt.getDeepseekConfigFile()
  ...
  const openSkillRoot = async (): Promise<void> => {
    ...
    if (typeof window.workgpt?.openSkillRoot !== 'function') return
    setSkillNotice(null)
    const result = await window.workgpt.openSkillRoot(selectedSkillRoot.path)
    if (!result.ok) {
  ...
  const saveMcpConfig = async (): Promise<void> => {
    if (typeof window.workgpt?.setDeepseekConfigFile !== 'function') return
    setMcpBusy(true)
    setMcpNotice(null)
    try {
      const result = await window.workgpt.setDeepseekConfigFile(mcpConfigText)
  ...
  const openMcpConfigDir = async (): Promise<void> => {
    if (typeof window.workgpt?.openDeepseekConfigDir !== 'function') return
    const result = await window.workgpt.openDeepseekConfigDir()
    if (!result.ok) {
  ...
  const pickWorkspace = async (): Promise<void> => {
    try {
      ...
      const picked = await window.workgpt.pickWorkspaceDirectory(form.workspaceRoot || undefined)
  ...
  const pickWriteWorkspace = async (): Promise<void> => {
    try {
      ...
      const picked = await window.workgpt.pickWorkspaceDirectory(
  ...
  const clearWriteDebugEntries = async (): Promise<void> => {
    ...
    setWriteDebugError(null)
    try {
      if (typeof window.workgpt?.clearWriteInlineCompletionDebugEntries === 'function') {
        await window.workgpt.clearWriteInlineCompletionDebugEntries()
      }
