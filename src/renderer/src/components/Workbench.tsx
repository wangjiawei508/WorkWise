...
import type { DesktopCommand, SkillListItem } from '@shared/workgpt-api'
...
export function Workbench(): ReactElement {
  ...
  useEffect(() => {
    const runDesktopShortcut = (command: DesktopCommand): void => {
      if (typeof window.workgpt?.runDesktopCommand !== 'function') return
      void window.workgpt.runDesktopCommand(command)
    }
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || typeof window.workgpt?.getLogPath !== 'function') return
    let cancelled = false
    void window.workgpt
      .getLogPath()
      .then((path) => {
  ...
  const mirrorClawCommand = async (userText: string, replyText: string): Promise<void> => {
    if (!activeThreadId || typeof window.workgpt?.mirrorClawChannelMessage !== 'function') return
    const userResult = await window.workgpt.mirrorClawChannelMessage(
      activeThreadId,
      userText,
      'user'
    )
    if (!userResult.ok) return
    await window.workgpt.mirrorClawChannelMessage(
      activeThreadId,
      replyText,
      'assistant'
    )
  ...
  useEffect(() => {
    ...
    const localSkillsTask = typeof window !== 'undefined' && typeof window.workgpt?.listSkills === 'function'
      ? window.workgpt.listSkills(activeSkillWorkspace || undefined)
  ...
  const handlePasteClipboardImage = async (options: { silentNoImage?: boolean } = {}): Promise<void> => {
    ...
    if (typeof window.workgpt?.readClipboardImage !== 'function') {
      setAttachmentUploadError(t('composerAttachmentUnavailable'))
      return
    }
    const image = await window.workgpt.readClipboardImage()
    if (!image.ok) {
  ...
  const startNewSddRequirement = async (): Promise<void> => {
    ...
    const restored = await restoreRememberedSddDraft({
      ...
      readWorkspaceFile: window.workgpt.readWorkspaceFile
    })
    ...
    const result = await window.workgpt.createWorkspaceFile({
      workspaceRoot: targetWorkspace,
  ...
  const readComposerFileContextEntries = async (
    ...
    for (const reference of references) {
      ...
      const result = await window.workgpt.readWorkspaceFile({
  ...
  const handleSendAsync = async (): Promise<void> => {
    ...
    if (route === 'claw') {
      ...
      setInput('')
      void (async () => {
        const taskResult = typeof window.workgpt?.createClawTaskFromText === 'function'
          ? await window.workgpt.createClawTaskFromText(v, {
  ...
  const renderRuntimeBanner = (message: string, detail?: string | null): ReactElement => (
    <RuntimeBanner
      ...
      t={t}
      onOpenLogDir={
        typeof window !== 'undefined' && typeof window.workgpt?.openLogDir === 'function'
          ? () => window.workgpt.openLogDir()
          : undefined
      }
