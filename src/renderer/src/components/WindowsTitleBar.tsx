...
function currentPlatform(): string {
  return typeof window !== 'undefined' ? window.workgpt?.platform ?? 'unknown' : 'unknown'
}

function defaultRunDesktopCommand(command: DesktopCommand): Promise<void> {
  if (typeof window === 'undefined' || typeof window.workgpt?.runDesktopCommand !== 'function') {
    return Promise.resolve()
  }
  return window.workgpt.runDesktopCommand(command)
}

function defaultOpenLogDir(): Promise<void> {
  if (typeof window === 'undefined' || typeof window.workgpt?.openLogDir !== 'function') {
    return Promise.resolve()
  }
...
export function WindowsTitleBar({ platform, actions }: Props): ReactElement | null {
  ...
  const defaultActions = useMemo<WindowsTitleBarActions>(() => ({
    ...
    showAbout: async () => {
      const version =
        typeof window !== 'undefined' && typeof window.workgpt?.getAppVersion === 'function'
          ? await window.workgpt.getAppVersion().catch(() => '')
          : ''
  ...
  return (
    <div ref={rootRef} className="ds-windows-titlebar ds-drag">
      <div className="ds-windows-titlebar-content">
        <img src={workgptLogo} alt="" aria-hidden="true" className="ds-windows-titlebar-icon" />
