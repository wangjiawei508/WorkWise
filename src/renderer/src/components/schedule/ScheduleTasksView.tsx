...
export function ScheduleTasksView({
  ...
  const load = useCallback(async (): Promise<void> => {
    try {
      const [nextSettings, nextStatus] = await Promise.all([
        ...
        typeof window.workgpt?.getScheduleStatus === 'function'
          ? window.workgpt.getScheduleStatus()
  ...
  const persistSchedule = async (patch: Parameters<typeof mergeScheduleSettings>[1]): Promise<void> => {
    ...
    setSettings(saved)
    if (typeof window.workgpt?.getScheduleStatus === 'function') {
      setStatus(await window.workgpt.getScheduleStatus())
    }
  }
  ...
  const pickDialogWorkspace = async (): Promise<void> => {
    ...
    try {
      ...
      const picked = await window.workgpt.pickWorkspaceDirectory(resolveDialogWorkspaceRoot(dialog.draft.workspaceRoot) || undefined)
  ...
  const runTask = async (taskId: string): Promise<void> => {
    if (typeof window.workgpt?.runScheduleTask !== 'function') return
    const result = await window.workgpt.runScheduleTask(taskId)
    if (!result.ok) {
