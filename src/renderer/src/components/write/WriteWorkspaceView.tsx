...
export function WriteWorkspaceView({
  ...
  const submitInlineEdit = async (prompt: string): Promise<void> => {
    ...
    if (typeof window.workgpt?.requestWriteInlineCompletion !== 'function') {
    ...
    try {
      const result = await window.workgpt.requestWriteInlineCompletion(
  ...
  const exportCurrentFile = async (format: WriteExportFormat): Promise<void> => {
    ...
    if (typeof window.workgpt?.exportWriteDocument !== 'function') {
    ...
    try {
      const result = await window.workgpt.exportWriteDocument({
        path: activeFilePath,
        workspaceRoot,
        format,
        content: fileContent
  ...
  const copyCurrentFileAsRichText = async (): Promise<void> => {
    if (!activeFilePath) return
    if (!activeFileIsText) return
    if (typeof window.workgpt?.copyWriteDocumentAsRichText !== 'function') {
    ...
    try {
      const result = await window.workgpt.copyWriteDocumentAsRichText({
        path: activeFilePath,
        workspaceRoot,
        content: fileContent
      })
  ...
  useEffect(() => {
    ...
    if (
      typeof window.workgpt?.watchWorkspaceFile !== 'function' ||
      typeof window.workgpt?.unwatchWorkspaceFile !== 'function' ||
      typeof window.workgpt?.onWorkspaceFileChanged !== 'function'
    ) {
      return
    }
