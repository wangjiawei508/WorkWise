...
export const writeMarkdownHardenOptions = {
  defaultOrigin: 'https://workgpt.local',
  allowedLinkPrefixes: ['*'],
...
function ResolvedMarkdownImage({
  ...
  useEffect(() => {
    ...
    if (!localPath || typeof window.workgpt?.readWorkspaceImage !== 'function') return

    void window.workgpt.readWorkspaceImage({ path: localPath })
      .then((result) => {
        if (cancelled) return
        if (result.ok) {
          ...
          a: ({ href, children, ...props }): ReactNode => (
            <a
              ...
              onClick={(event) => {
                ...
                void window.workgpt?.openExternal?.(href)?.catch(() => undefined)
              }}
            >
              {children}
            </a>
