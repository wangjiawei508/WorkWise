...
export function ConnectPhoneView({
  ...
  const startOfficialInstallQr = async (): Promise<void> => {
    ...
    if (
      ...
      typeof window.workgpt?.startClawImInstallQr !== 'function'
    ) {
      setInstallQr({
        ...INITIAL_QR_STATE,
        status: 'error',
    ...
    try {
      result = await window.workgpt.startClawImInstallQr(request.provider, request.options)
    } catch (error) {
    ...
    const waitForInstall = async (): Promise<void> => {
      try {
        if (
          typeof window === 'undefined' ||
          typeof window.workgpt?.pollClawImInstall !== 'function'
        ) {
          throw new Error(t('clawAddImOfficialQrUnavailable'))
        }
        const poll = await window.workgpt.pollClawImInstall(request.provider, result.deviceCode)
        if (installAttempt !== installAttemptRef.current) return
        if (poll.done) {
          clearInstallTimers()
          setInstallQr((current) => ({
