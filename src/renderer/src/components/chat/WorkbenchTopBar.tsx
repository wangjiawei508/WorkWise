...
export function WorkbenchTopBar({
  ...
  useEffect(() => {
    let cancelled = false
    if (typeof window.workgpt?.listEditors !== 'function') return

    void window.workgpt.listEditors()
      .then((result) => {
        if (cancelled) return
  ...
  useEffect(() => {
    if (typeof window.workgpt?.onGuiUpdateState !== 'function') return
    const applyState = (state: GuiUpdateState): void => {
      setGuiUpdateState(state)
    }
    const unsubscribe = window.workgpt.onGuiUpdateState(applyState)
    if (typeof window.workgpt?.getGuiUpdateState === 'function') {
      void window.workgpt.getGuiUpdateState().then(applyState).catch(() => undefined)
    }
    return unsubscribe
  }, [])
  ...
  const runGuiUpdateAction = async (): Promise<void> => {
    ...
    if (guiUpdateAction.manualOnly) {
      if (typeof window.workgpt?.openExternal === 'function') {
        await window.workgpt.openExternal(guiUpdateAction.releaseUrl)
      }
      return
    }
    if (
      typeof window.workgpt?.downloadGuiUpdate !== 'function' ||
      typeof window.workgpt?.installGuiUpdate !== 'function'
    ) {
      return
    }
    ...
    try {
      if (!guiUpdateAction.downloaded && guiUpdateState.status !== 'downloaded') {
        const downloadResult = await window.workgpt.downloadGuiUpdate(guiUpdateAction.channel)
        if (!downloadResult.ok) return
      }
      const installResult = await window.workgpt.installGuiUpdate()
      if (!installResult.ok && typeof window.workgpt?.logError === 'function') {
        await window.workgpt.logError('gui-update', 'Failed to install GUI update from workbench top bar', {
          version: guiUpdateAction.latestVersion,
          message: installResult.message
        })
      }
    } catch (error) {
      if (typeof window.workgpt?.logError === 'function') {
        await window.workgpt.logError('gui-update', 'Failed to apply GUI update from workbench top bar', {
          version: guiUpdateAction.latestVersion,
