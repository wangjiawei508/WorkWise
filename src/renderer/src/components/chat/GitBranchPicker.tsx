...
export function GitBranchPicker({ workspaceRoot }: Props): ReactElement | null {
  ...
  const load = useCallback(async (): Promise<void> => {
    if (!root || typeof window.workgpt?.getGitBranches !== 'function') return
    setLoading(true)
    setError(null)
    try {
      const next = await window.workgpt.getGitBranches(root)
      setResult(next)
  ...
  const switchBranch = async (branch: string): Promise<void> => {
    ...
    setError(null)
    try {
      const next = await window.workgpt.switchGitBranch(root, branch)
      setResult(next)
      if (!next.ok) {
  ...
  const createBranch = async (): Promise<void> => {
    ...
    setError(null)
    try {
      const next = await window.workgpt.createAndSwitchGitBranch(root, branch)
      setResult(next)
      if (!next.ok) {
        setError(next.message)
        return
