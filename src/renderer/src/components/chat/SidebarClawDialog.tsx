...
import type { ClawImInstallQrResult } from '@shared/workgpt-api'
import {
  ClawProviderLogo,
...
export function ClawAddImDialog({
  ...
  useEffect(() => {
    let cancelled = false
    if (typeof window.workgpt?.getSettings !== 'function') return
  ...
  const startOfficialInstallQr = async (): Promise<void> => {
    ...
    if (typeof window.workgpt?.startClawImInstallQr !== 'function') {
      setInstallQr({
        status: 'error',
        url: '',
        deviceCode: '',
    ...
    try {
      result = await window.workgpt.startClawImInstallQr(officialInstallProvider, {
    ...
    const waitForInstall = async (): Promise<void> => {
      try {
        const poll = await window.workgpt.pollClawImInstall(officialInstallProvider, result.deviceCode)
        if (installAttempt !== installAttemptRef.current) return
        if (poll.done) {
          clearInstallTimers()
