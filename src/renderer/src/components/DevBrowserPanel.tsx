...
const PREVIEW_URL_STORAGE_KEY = 'workgpt.devPreview.url'
const PREVIEW_AUTO_FOLLOW_STORAGE_KEY = 'workgpt.devPreview.autoFollow'
...
export function DevBrowserPanel({
  ...
  const useElectronWebview = canUseElectronWebviewEnvironment({
    openExternalAvailable: typeof window.workgpt?.openExternal === 'function',
  ...
  const openExternal = (): void => {
    ...
    if (typeof window.workgpt?.openExternal === 'function') {
      void window.workgpt.openExternal(normalized).catch(() => undefined)
      return
    }
