...
export function useSettingsGuiUpdate({
  ...
  category: 'general' | 'write' | 'agents' | 'shortcuts' | 'claw' | 'help'
  ...
  const checkGuiUpdate = useCallback(async (): Promise<void> => {
    if (typeof window.workgpt?.checkGuiUpdate !== 'function') return
    setCheckingGuiUpdate(true)
    setGuiUpdateError(null)
    try {
      const info = await window.workgpt.checkGuiUpdate(channel)
  ...
  const downloadGuiUpdate = async (): Promise<void> => {
    if (typeof window.workgpt?.downloadGuiUpdate !== 'function') return
    setDownloadingGuiUpdate(true)
    setGuiUpdateProgress(null)
    setGuiUpdateError(null)
    try {
      const result = await window.workgpt.downloadGuiUpdate(form?.guiUpdate?.channel)
      if (!result.ok) {
  ...
  const installGuiUpdate = async (): Promise<void> => {
    if (typeof window.workgpt?.installGuiUpdate !== 'function') return
    setInstallingGuiUpdate(true)
    setGuiUpdateError(null)
    try {
      const result = await window.workgpt.installGuiUpdate()
      if (!result.ok) {
        setGuiUpdateError(result.message)
        setInstallingGuiUpdate(false)
      }
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.workgpt.onGuiUpdateState(applyGuiUpdateState)
    if (typeof window.workgpt?.getGuiUpdateState === 'function') {
      void window.workgpt.getGuiUpdateState().then(applyGuiUpdateState).catch(() => undefined)
    }
    return unsubscribe
  }, [applyGuiUpdateState])
