...
function SurveyResultSummary({ adjustment, onAskPoint }: { adjustment: Adjustment | null; onAskPoint?: (pointId: string) => void }): ReactElement | null {
  ...
  return <section className="survey-result-summary mt-3 border border-ds-border-muted bg-ds-card" aria-label={t('surveyPointResults')}>
    ...
    <div className="overflow-x-auto"><table className="min-w-full text-left text-[10.5px]"><thead className="bg-ds-subtle text-ds-muted"><tr><th className="px-3 py-2 font-semibold">{t('surveyPointId')}</th><th className="px-3 py-2 font-semibold">{t('surveyAdjustedX', { unit: linearUnit })}</th><th className="px-3 py-2 font-semibold">{t('surveyAdjustedY', { unit: linearUnit })}</th><th className="px-3 py-2 font-semibold">{t('surveyLatitudeLongitude')}</th><th className="px-3 py-2 font-semibold">{t('surveyAdjustedHeight', { unit: linearUnit })}</th><th className="px-3 py-2 font-semibold">{t('surveyCorrection', { unit: linearUnit })}</th><th className="px-3 py-2 font-semibold">{t('surveyPointError', { unit: linearUnit })}</th></tr></thead><tbody className="divide-y divide-ds-border-muted">{pointRows.map((point) => <tr key={point.id} className="hover:bg-ds-hover"><td className="px-3 py-2.5 font-medium text-ds-ink">{point.id}<button type="button" disabled={!onAskPoint} aria-label={t('surveyAskEvidence', { label: point.id })} onClick={() => onAskPoint?.(point.id)} className="mt-1 block text-accent disabled:opacity-50">{t('surveyAskAgent')}</button></td><td className="px-3 py-2.5 tabular-nums text-ds-ink">{numberLabel(point.x, 6)}</td><td className="px-3 py-2.5 tabular-nums text-ds-ink">{numberLabel(point.y, 6)}</td><td className="px-3 py-2.5 tabular-nums text-ds-ink">{point.latitude === undefined && point.longitude === undefined ? '—' : `${numberLabel(point.latitude, 8)} / ${numberLabel(point.longitude, 8)}`}</td><td className="px-3 py-2.5 tabular-nums text-ds-ink">{numberLabel(point.height, 6)}</td><td className="px-3 py-2.5 tabular-nums text-ds-muted">{numberLabel(point.correctionHeight ?? point.correctionX ?? point.correctionY, 7)}</td><td className="px-3 py-2.5 tabular-nums text-ds-muted">{numberLabel(point.standardError, 6)}</td></tr>)}</tbody></table></div>
  </section>
}
