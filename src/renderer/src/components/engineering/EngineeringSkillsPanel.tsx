...
function skillStatus(skill: Skill): { label: string; tone: string } {
  if (skill.status === 'available' && skill.packaged) return { label: '已审计 · 已打包', tone: 'text-green-700 dark:text-green-300' }
  if (skill.status === 'blocked') return { label: '审查阻断 · 未打包', tone: 'text-red-700 dark:text-red-300' }
  return { label: '审查中 · 未打包', tone: 'text-amber-700 dark:text-amber-300' }
}
