...
const INSTALLED_STORAGE_KEY = 'workgpt.installedPlugins'
const ACTIVE_KIND_STORAGE_KEY = 'workgpt.marketplace.kind'
...
export function PluginMarketplaceView(): ReactElement {
  ...
  const readMcpConfig = useCallback(async (): Promise<string> => {
    if (typeof window.workgpt?.getDeepseekConfigFile !== 'function') return mcpConfigText
    const file = await window.workgpt.getDeepseekConfigFile()
  ...
  const refreshMcpRuntimeOverlay = useCallback(async (): Promise<void> => {
    if (typeof window.workgpt?.runtimeRequest !== 'function') {
      setRuntimeInfo(null)
  ...
  const refreshSkillList = useCallback(async (
    ...
    if (typeof window.workgpt?.listSkills !== 'function') {
      setDiscoveredSkills([])
    ...
    try {
      let syncError = ''
      if (options.syncOnline && typeof window.workgpt?.syncGithubSkills === 'function') {
  ...
  const appendMcpConfig = async (
    ...
    const result = await window.workgpt.setDeepseekConfigFile(merged.text)
  ...
  const addItem = async (item: MarketplaceItem): Promise<void> => {
    ...
    try {
      ...
      const result = await window.workgpt.saveSkillFile(selectedSkillRoot.path, item.id, content)
      if (!result.ok) {
        setNotice({ tone: 'error', message: result.message })
        return
      }
  ...
  const addCustom = async (): Promise<void> => {
    ...
    try {
      if (activeKind === 'mcp') {
        ...
        const result = await window.workgpt.saveSkillFile(selectedSkillRoot.path, id, content)
        if (!result.ok) {
          setNotice({ tone: 'error', message: result.message })
          return
        }
...
function runtimeOverlayErrorMessage(error: unknown, fallback: string): string {
  const message = error instanceof Error ? error.message : String(error)
  return /runtimeRequest|workgpt|Cannot read properties/i.test(message) ? fallback : message
}
