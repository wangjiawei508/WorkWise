...
function implicitResolvedModelOverride(settings: Awaited<ReturnType<typeof rendererRuntimeClient.getSettings>>): string | undefined {
  const rawModel = getKunRuntimeSettings(settings).model.trim()
  const resolvedModel = resolveKunRuntimeSettings(settings).model.trim()
  return resolvedModel && resolvedModel !== rawModel ? resolvedModel : undefined
}
...
export class KunRuntimeProvider implements AgentProvider {
  ...
  async archiveThread(threadId: string, archived: boolean): Promise<void> {
    const response = await window.workgpt.runtimeRequest(
      kunThreadPath(threadId),
      'PATCH',
