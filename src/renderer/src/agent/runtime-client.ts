...
class RendererRuntimeClient {
  ...
  async getSettings(options?: { forceRefresh?: boolean }): Promise<AppSettingsV1> {
    ...
    const task = window.workgpt.getSettings().then((settings) => {
  ...
  async setSettings(partial: AppSettingsPatch): Promise<AppSettingsV1> {
    const settings = await window.workgpt.setSettings(partial)
    this.cachedSettings = settings
  ...
  runtimeRequest(path: string, method?: string, body?: string): Promise<RuntimeRequestResult> {
    if (body === undefined) {
      if (method === undefined) return window.workgpt.runtimeRequest(path)
      return window.workgpt.runtimeRequest(path, method)
    }
    return window.workgpt.runtimeRequest(path, method, body)
  }

  startSse(threadId: string, sinceSeq: number, streamId?: string): Promise<{ streamId: string }> {
    return window.workgpt.startSse(threadId, sinceSeq, streamId)
  }

  stopSse(streamId: string): Promise<boolean> {
    return window.workgpt.stopSse(streamId)
  }

  onSseEvent(handler: (payload: SseEventPayload) => void): () => void {
    return window.workgpt.onSseEvent(handler)
  }

  onSseEnd(handler: (payload: SseEndPayload) => void): () => void {
    return window.workgpt.onSseEnd(handler)
  }

  onSseError(handler: (payload: SseErrorPayload) => void): () => void {
    return window.workgpt.onSseError(handler)
  }
}
