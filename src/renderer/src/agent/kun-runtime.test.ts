...
function installWorkgpt(overrides: Partial<Window['workgpt']>): void {
...
describe('KunRuntimeProvider', () => {
  ...
  it('creates threads with the resolved provider model when Kun still has the default model', async () => {
    ...
    customSettings.agents.kun.apiType = 'chat_completions'
    ...
    await provider.createThread({ workspace: '/tmp/workspace', title: 'New chat', mode: 'agent' })
  ...
  it('posts the resolved provider model for turns when Kun still has the default model', async () => {
    const customSettings = settings()
    customSettings.provider.providers = [
      {
        ...customSettings.provider.providers[0],
  ...
  it('posts GUI plan context with Kun plan turn requests', async () => {
    ...
    await provider.sendUserMessage('thr_1', 'refine the plan', {
      ...
      guiPlan: {
        operation: 'refine',
        workspaceRoot: '/workspace/workgpt',
        relativePath: '.kunsdd/plan/auth.md',
        planId: '/workspace/workgpt:.kunsdd/plan/auth.md',
