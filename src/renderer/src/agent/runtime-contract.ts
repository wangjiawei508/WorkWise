...
export type CoreRuntimeCapabilityManifestJson = {
  ...
  /** Optional so the GUI keeps working against older WorkWise Runtime builds without the capability. */
...
export type CoreRuntimeInfoJson = {
  ...
  resourceLimits?: import('@shared/runtime-resource-limits').RuntimeResourceLimitsV1
}
 ...
 * `create_plan` tool result. Mirrors the WorkWise Runtime output contract
 ...
 * reserved plan id, workspace root, and relative path the WorkWise Runtime
...
/**
 * Native WorkWise Runtime plan tool name. Re-exported alongside the shared
