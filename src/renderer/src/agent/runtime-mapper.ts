...
function reviewTargetFromCore(target: CoreReviewTargetJson | undefined): ReviewTarget | undefined {
  ...
  switch (target.kind) {
    case 'uncommittedChanges':
      return { kind: 'uncommittedChanges', repositoryRoot: target.repositoryRoot }
    case 'baseBranch':
      return target.branch?.trim()
        ? { kind: 'baseBranch', branch: target.branch, repositoryRoot: target.repositoryRoot }
        : undefined
    case 'commit':
      return target.sha?.trim()
        ? { kind: 'commit', sha: target.sha, repositoryRoot: target.repositoryRoot }
        : undefined
    case 'custom':
      return target.instructions?.trim()
        ? { kind: 'custom', instructions: target.instructions, repositoryRoot: target.repositoryRoot }
        : undefined
    default:
      return undefined
  }
 ...
 */
export async function dispatchRuntimeEvents(
  ...
  for (const event of events) {
    ...
    flushDeltas()
    await dispatchRuntimeEvent(event, sink, handleApprovalRequest)
  }
  flushDeltas()
}
