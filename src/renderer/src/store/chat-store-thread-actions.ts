  ...
  sendMessage: async (text, mode, overrides) => {
    ...
    if (!activeThreadId) {
      try {
        ...
        void window.workgpt.logError('create-thread', 'Failed to create thread', {
    ...
    try {
      ...
      if (channel && typeof window.workgpt?.mirrorClawChannelMessage === 'function') {
        const userMirror = await window.workgpt.mirrorClawChannelMessage(
          activeThreadId,
          trimmedText,
          'user'
        )
      ...
      clearBusyWatchdog()
      void window.workgpt.logError('send-message', 'Failed to send message', {
        message: e instanceof Error ? e.message : String(e),
        threadId: activeThreadId
      }).catch(() => undefined)
