...
export function createMaintenanceActions(
  ...
  resolveApproval: async (blockId, decision) => {
    ...
    try {
      ...
      void window.workgpt.logError('approval', 'Failed to submit approval decision', {
        message: msg,
        blockId
      }).catch(() => undefined)
      set((s) => ({
  ...
  resolveUserInput: async (blockId, action) => {
    ...
    try {
      ...
      const msg = formatRuntimeError(e)
      void window.workgpt.logError('user-input', 'Failed to resolve user input', {
        message: msg,
        blockId
      }).catch(() => undefined)
      set((s) => ({
  ...
  interrupt: async (options) => {
    ...
    try {
      ...
      const msg = formatRuntimeError(e)
      void window.workgpt.logError('interrupt', 'Failed to interrupt turn', { message: msg }).catch(() => undefined)
      set({
        error: msg,
        ...(shouldOpenSettingsForError(e)
          ? { route: 'settings' as const, settingsSection: 'agents' as const }
