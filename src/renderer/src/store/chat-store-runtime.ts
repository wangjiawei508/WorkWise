...
function notifyTurnComplete(threadId: string | null, state: ChatState, dedupeKey: string): void {
  if (!threadId || typeof window.workgpt?.showTurnCompleteNotification !== 'function') return
  ...
  void window.workgpt
    ...
    .then((result) => {
      if (result.ok || typeof window.workgpt?.logError !== 'function') return
      void window.workgpt.logError('notification', 'Turn completion notification failed', {
        message: result.message,
        threadId
      }).catch(() => undefined)
    })
    .catch((error: unknown) => {
      if (typeof window.workgpt?.logError !== 'function') return
      void window.workgpt.logError('notification', 'Turn completion notification failed', {
...
export function buildThreadEventSink(
  ...
  return {
    ...
    onTurnComplete: () => {
      ...
      if (pendingMirror && assistantMirrorText && typeof window.workgpt?.mirrorClawChannelMessage === 'function') {
        void window.workgpt.mirrorClawChannelMessage(
