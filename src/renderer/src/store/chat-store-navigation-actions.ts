...
export function createNavigationActions(
  ...
  probeRuntime: async (mode = 'user') => {
    ...
    try {
      if (typeof window.workgpt === 'undefined') {
        throw new Error(
          'Preload bridge missing (window.workgpt). Restart the app or check BrowserWindow preload path.'
        )
      }
      ...
      try {
        if (typeof window.workgpt === 'undefined') {
          set({
            ...
            runtimeConnection: 'offline',
            runtimeErrorDetail: 'Preload bridge missing (window.workgpt). Restart the app or check BrowserWindow preload path.',
        ...
        if (!clawChannelActivityUnsubscribe && typeof window.workgpt.onClawChannelActivity === 'function') {
          clawChannelActivityUnsubscribe = window.workgpt.onClawChannelActivity(({ channelId, threadId }) => {
            void (async () => {
              const state = get()
              if (typeof window.workgpt === 'undefined') return
              const settings = await rendererRuntimeClient.getSettings({ forceRefresh: true })
  ...
  chooseWorkspace: async ({ createThreadAfter = false, selectThreadAfter = true } = {}) => {
    try {
      ...
      if (typeof window.workgpt === 'undefined' || typeof window.workgpt.pickWorkspaceDirectory !== 'function') {
        throw new Error(i18n.t('common:workspacePickerUnavailable'))
      }
      const picked = await window.workgpt.pickWorkspaceDirectory(get().workspaceRoot || undefined)
  ...
  clearWorkspace: async () => {
    try {
      if (typeof window.workgpt === 'undefined' || typeof window.workgpt.setSettings !== 'function') {
        return
      }
  ...
  deleteWorkspace: async (workspacePath) => {
    ...
    try {
      ...
      if (normalizeWorkspaceRoot(get().workspaceRoot) === normalizedPath) {
        try {
          if (typeof window.workgpt?.setSettings === 'function') {
            const next = await rendererRuntimeClient.setSettings({ workspaceRoot: '' })
            set({
              workspaceRoot: normalizeWorkspaceRoot(next.workspaceRoot),
              codeWorkspaceRoots: get().codeWorkspaceRoots,
