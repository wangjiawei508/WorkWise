...
function channel(overrides: Partial<ClawImChannelV1> = {}): ClawImChannelV1 {
  ...
  return {
    ...
    workspaceRoot: '/Users/zxy/.workgpt/claw/agent01',
    agentProfile: {
      name: '',
      description: '',
      identity: '',
    ...
    conversations: [
      {
        ...
        workspaceRoot: '/Users/zxy/.workgpt/claw/agent01/conversations/chat-1',
        createdAt: now,
        updatedAt: now
      }
    ],
...
function thread(id: string, title: string, updatedAt = '2026-06-01T00:00:00.000Z'): NormalizedThread {
  return {
    ...
    model: 'reasonix',
    mode: 'agent',
    workspace: '/Users/zxy/.workgpt/default_workspace'
  }
}
...
describe('chat-store Claw actions helpers', () => {
  ...
  it('recovers an unmapped Claw managed Kun session before creating a new empty one', () => {
    ...
    const recovered = findRecoverableClawThread(
      [
        ...
        thread('old-content-thread', `${CLAW_MANAGED_INSTRUCTIONS_HEADING} WORKGPT scheduled-task tools`, '2026-06-01T00:01:00.000Z')
      ],
      [item],
      item
    )
  ...
  it('keeps an empty IM channel on the Claw route instead of selecting a stale missing thread', async () => {
    ...
    expect(workgpt.setSettings).toHaveBeenCalledWith({
      claw: {
