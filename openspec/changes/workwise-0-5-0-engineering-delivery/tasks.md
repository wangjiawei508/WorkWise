...
Typed resume UI followthrough: stalled/waiting tasks continue through the existing plan-bound route with model/provider/effort captured once, duplicate-click protection and scope/revision checks. Failed/cancelled tasks require explicit replanning and fresh approval. Runtime also compensates failed starts and plan persistence without overwriting concurrent progress or leaving an unlaunched turn running. Desktop full tests pass 2807/2 skipped, Runtime 2819/22 skipped; typechecks, build, strict OpenSpec and lint pass (1 existing warning). The 9cf package rejects zero-tool false completion and projects unchanged legacy records as needing attention, but does not include these later continuation fixes. Actual packaged stall-to-resume acceptance remains part of the unchecked exact-package task above.

Exact 53213de candidate evidence is now retained in `docs/qa/evidence/railwise-convergence-53213de739d0/README.md`. The signed/notarized package passed its private native updater round trip. Seven text-only mock replies stalled the original Task with four absent receipts. The first real resume failed on upstream HTTP 503 without tools or outputs; that Task and history remain failed and unchanged. After explicit GUI replan and fresh approval, a second scenario repeated the mock probe, then resumed its own original Task with the same provider/model/off selection: five successful real chats, four typed tools and verified receipts, and three real output files. Independent numerical comparison and normal-restart record/file hash comparison passed. Ordinary GUI consultation was rejected locally by the acceptance relay catalog guard; a later minimal CLI health question is not GUI consultation acceptance.

The same 53213de package still displays an obsolete waiting reason after completion, so final UI acceptance remains failed and the unchecked aggregate gates stay unchecked. The separate four-file source fix at `7d4f454feecb09028007ae9b436a318db6856201` clears completed-task reasons and scopes diagnostics without deleting historical events. Desktop 2818 passed/2 skipped, Runtime 2821 passed/22 skipped, build/typechecks/strict OpenSpec passed, and lint has 0 errors with 1 existing warning; it requires a new signed-package UI check. No public release, production KPI claim, professional sign-off or private-data publication is implied.

The P0 before/after-restart public summaries in the same archive verify two real-format networks with the same eight identities, inputs, algorithm and six output hashes/sizes/types, preserving draft status. Each network has two bound started/finished verification attempts, both passed and none incomplete. The parent agent observed normal exit, process absence, same-package restart and five successful GUI checks on each original manifest; the read-only summary does not independently certify those GUI actions. IN2 independent OU2 comparison covers 33 points; GSI same-source independent adjustment covers 21 unknowns. Historical GSI/IN1 conversion differences remain unresolved for 7 of 14 route heights at half-printed-unit precision. Private coordinates, source paths, screenshots and detailed records remain outside the repository; aggregate gates remain unchecked.
