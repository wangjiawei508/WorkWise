...
这一版是 WORKGPT 的一次全面改版。

WORKGPT 不再只是一个聊天窗口，而是一个围绕真实项目工作的本地 AI 工作台：Code 处理代码库，Write 打磨文档，连接手机接入 IM 与定时任务，需求、计划、目标、代码审查、Skill/MCP 和更新都被收进同一个图形化流程里。

### 内置 Kun Agent 运行时

本版最大的变化，是 WORKGPT 现在以内置自研 Agent 运行时 **Kun** 作为唯一核心运行时。
...
这是 WORKGPT 现在很重要的一条产品范式：Agent 不再只是在聊天框里回答问题，而是和 GUI 一起承载需求、计划、执行和复盘。
...
这让 WORKGPT 不只是在你打开聊天窗口时工作，也可以成为持续运行的本地自动化 Agent。
...
v0.2.0 是 WORKGPT 从“桌面聊天工具”走向“本地 Agent 工作台”的一次关键版本。

Kun 负责把模型调用、缓存、上下文和工具成本管住；GUI 负责把需求、计划、目标、审查、写作、自动化和文件改动变得可见、可控、可继续。两者结合之后，WORKGPT 更适合真实项目里的长期工作，也更能把每一个 token 用在刀刃上。
