...
0.3.1 正式版完成 Design 设计工作台、Write/PPT 联动与直接 PNG/SVG 交付，同时把 Write 的 Word 导出升级为模板驱动。本次发布沿用 WorkWise 0.2.5–0.3.0 的历史标准：macOS 安装包可以未签名、未公证，但必须完成源码、三平台打包、两小时稳定性和可执行候选验收，并在发布页明确安装提示。
...
- 最终三平台候选运行 `30057460266` 全部通过；候选 artifact 只含三个客户端和内部校验清单，正式发布也只向公开 Release 上传三个客户端。
- 最终客户端 SHA-256：
...
- 独立下载复核确认两个 DMG、Windows NSIS、ASAR、MarkItDown、PPT Master 4.0.0、原生 SQLite、版本和内部更新元数据均正确。
