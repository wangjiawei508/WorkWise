...
async function makeXlsx(project: RailwiseProjectV1, dataset: StoredDataset, analysis: MonitoringAnalysisV1, citations: KnowledgeCitationV1[], adjustments: AdjustmentResultV1[], runtimeVersion: string): Promise<Buffer> {
  ...
  const sheets: Array<{ name: string; rows: string[][] }> = [
    ...
    { name: 'survey_adjustments', rows: [['runId', 'networkId', 'resultId', 'strategyId', 'transformType', 'algorithmVersion', 'observationCount', 'unknownCount', 'redundancy', 'unitWeightStdDev', 'unitWeightStdDevUnit', 'maxPointStdDev', 'maxPointStdDevUnit', 'validation', 'inputHash'], ...adjustments.map((a) => [a.runId, a.networkId, a.id, a.strategyId ?? '', a.transformType ?? '', a.algorithmVersion, String(a.observationCount), String(a.unknownCount), String(a.redundancy), String(a.unitWeightStdDev), 'dimensionless', String(a.precision.maxPointStdDev), a.linearUnit, a.validation, a.inputHash])] },
