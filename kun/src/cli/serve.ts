...
export function parseServeOptions(
  ...
  const merged: ServeOptions = {
    ...
    apiKey:
      typeof raw['api-key'] === 'string'
        ...
        : typeof raw.apiKey === 'string'
          ? raw.apiKey
          : env.DEEPSEEK_API_KEY ?? env.OPENAI_API_KEY ?? configServe.apiKey ?? DEFAULT_SERVE_OPTIONS.apiKey,
    baseUrl:
    ...
    apiType:
      typeof raw['api-type'] === 'string'
        ? raw['api-type'] as ServeOptions['apiType']
        : typeof raw.apiType === 'string'
          ? raw.apiType as ServeOptions['apiType']
          : env.KUN_API_TYPE === 'responses'
            ? 'responses'
            : configServe.apiType ?? DEFAULT_SERVE_OPTIONS.apiType,
    model:
