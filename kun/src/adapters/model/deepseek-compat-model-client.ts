...
export type DeepseekCompatConfig = {
  ...
  apiKey: string
  model: string
  /** Provider API protocol. Defaults to Chat Completions for compatibility. */
...
export class DeepseekCompatModelClient implements ModelClient {
  ...
  async *stream(request: ModelRequest): AsyncIterable<ModelStreamChunk> {
    ...
    if (this.config.nonStreaming || response.headers.get('content-type')?.includes('application/json')) {
      ...
      if (useResponsesApi) {
        yield* this.materializeResponsesNonStreaming(json as ResponsesResponse)
      } else {
        yield* this.materializeNonStreaming(json as ChatCompletionResponse)
      }
      return
    }
    if (!response.body) {
    ...
    yield* this.streamSse(response.body, request.abortSignal, useResponsesApi)
  }
  ...
  private buildRequestBody(request: ModelRequest, stream: boolean): Record<string, unknown> {
    ...
    const includeThinking = isDeepSeekHost(this.config.baseUrl)
  ...
  private buildResponsesRequestBody(
    ...
    applyResponsesReasoningEffort(body, request.reasoningEffort)
    const tools = normalizeToolSpecs(request.tools)
    if (tools.length > 0) {
      body.tools = tools.map((tool) => ({
        type: 'function',
    ...
    try {
      while (!signal.aborted) {
        ...
        while ((boundary = buffer.indexOf('\n\n')) >= 0) {
          ...
          const result: StreamPayloadResult = responsesApi
            ? this.consumeResponsesStreamPayload(
          ...
          if (typeof result.sawCompletedToolCall === 'boolean') sawCompletedToolCall = result.sawCompletedToolCall
  ...
  private consumeResponsesStreamPayload(
    ...
    textAccumulator: string,
    reasoningAccumulator: string,
    sawCompletedToolCallAccumulator: boolean
  ): StreamPayloadResult {
    const chunks: ModelStreamChunk[] = []
    let text = textAccumulator
    let reasoning = reasoningAccumulator
    let finishReason: string | null = null
    let usage: UsageSnapshot | null = null
    let sawCompletedToolCall = sawCompletedToolCallAccumulator
    const type = typeof payload.type === 'string' ? payload.type : ''
    const delta = payload.delta
    const payloadError = payload.error as { message?: unknown; code?: unknown } | undefined
    if (type === 'error' || payloadError?.message) {
      ...
      chunks.push({ kind: 'error', message, code })
      finishReason = 'error'
    }
    if (typeof delta === 'string' && isResponsesTextDelta(type)) {
      text += delta
      chunks.push({ kind: 'assistant_text_delta', text: delta })
    }
    if (typeof delta === 'string' && isResponsesReasoningDelta(type)) {
      reasoning += delta
      chunks.push({ kind: 'assistant_reasoning_delta', text: delta })
    }
    if (typeof delta === 'string' && isResponsesFunctionArgumentsDelta(type)) {
      const id = responsesPendingToolCallId(payload, pendingArguments)
      const existing = pendingArguments.get(id) ?? { arguments: '' }
      existing.arguments += delta
      pendingArguments.set(id, existing)
      chunks.push({
        kind: 'tool_call_delta',
        callId: existing.callId ?? id,
        toolName: existing.name,
    ...
    if (item && typeof item === 'object' && item.type === 'function_call') {
      const id = responsesToolCallPendingKey(item, pendingArguments)
      const existing = pendingArguments.get(id) ?? { arguments: '' }
      existing.callId = item.call_id ?? existing.callId ?? id
      existing.name = item.name ?? existing.name
      if (typeof item.arguments === 'string') existing.arguments = item.arguments
      pendingArguments.set(id, existing)
      if (type.endsWith('.done') && existing.name) {
        chunks.push({
          kind: 'tool_call_complete',
          callId: existing.callId ?? id,
          toolName: existing.name,
        ...
        pendingArguments.delete(id)
        sawCompletedToolCall = true
      }
    }
    const response = payload.response as ResponsesResponse | undefined
    if (response?.usage) {
      usage = this.mapUsage(response.usage)
    }
    if (payload.usage && typeof payload.usage === 'object') {
      usage = this.mapUsage(payload.usage as Record<string, unknown>)
    }
    if (type === 'response.completed') {
      finishReason = pendingArguments.size > 0 || sawCompletedToolCall ? 'tool_calls' : 'stop'
      for (const [callId, value] of pendingArguments) {
        if (!value.name) continue
        chunks.push({
          kind: 'tool_call_complete',
          callId: value.callId ?? callId,
          toolName: value.name,
          arguments: this.parseToolArguments(value.arguments || '{}')
        })
        sawCompletedToolCall = true
      }
      pendingArguments.clear()
    } else if (type === 'response.incomplete') {
      finishReason = response?.incomplete_details?.reason === 'max_output_tokens' ? 'length' : 'error'
    } else if (type === 'response.failed') {
      const message = response?.error?.message ?? 'model response failed'
      chunks.push({ kind: 'error', message, code: response?.error?.code })
      finishReason = 'error'
    }
    return { chunks, text, reasoning, finishReason, usage, sawCompletedToolCall }
  }
  ...
  private *materializeResponsesNonStreaming(
    ...
    if (payload.error?.message) {
      yield { kind: 'error', message: payload.error.message, code: payload.error.code }
      return
    }
      ...
      if (item.type === 'message' && Array.isArray(item.content)) {
        for (const part of item.content) {
          ...
          if (part.type === 'reasoning_text' || part.type === 'summary_text') {
            yield { kind: 'assistant_reasoning_delta', text: part.text }
          } else {
            yield { kind: 'assistant_text_delta', text: part.text }
          }
        }
        ...
        yield {
          kind: 'tool_call_complete',
          callId: item.call_id ?? item.id ?? `call_${output.indexOf(item) + 1}`,
          toolName: item.name,
    ...
    let stopReason: 'stop' | 'tool_calls' | 'length' | 'error' = 'stop'
    if (output.some((item) => item.type === 'function_call')) stopReason = 'tool_calls'
    else if (payload.incomplete_details?.reason === 'max_output_tokens') stopReason = 'length'
    else if (payload.status === 'failed') stopReason = 'error'
    yield { kind: 'completed', stopReason }
  }

  private mapUsage(usage: Record<string, unknown>): UsageSnapshot {
    const promptTokens = Number(usage.prompt_tokens ?? usage.input_tokens ?? usage.prompt_eval_count ?? 0) || 0
    const completionTokens = Number(usage.completion_tokens ?? usage.output_tokens ?? usage.eval_count ?? 0) || 0
...
function applyResponsesReasoningEffort(
  ...
  const normalized = effort?.trim().toLowerCase()
  if (!normalized) return
  const mapped = mapResponsesReasoningEffort(normalized)
  if (!mapped) return
...
function messageToResponsesInputItem(message: ChatMessage): unknown {
  ...
  const items: unknown[] = []
  const content = messageContentToResponsesContent(message.role, message.content)
...
function isResponsesTextDelta(type: string): boolean {
...
function isResponsesFunctionArgumentsDelta(type: string): boolean {
  return type === 'response.function_call_arguments.delta'
}

function responsesPendingToolCallId(
  payload: Record<string, unknown>,
  ...
  const existing = findPendingToolCallIdByIndex(pending, outputIndex)
  return existing ?? `call_${pending.size + 1}`
}

function responsesToolCallPendingKey(
  ...
  if (item.call_id) {
    for (const [key, value] of pending) {
      if (value.callId === item.call_id) return key
    }
    return item.call_id
  }
  return `call_${pending.size + 1}`
}

function flattenResponsesInputItems(items: unknown[]): unknown[] {
