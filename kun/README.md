...
Kun is the local HTTP/SSE agent runtime for WORKGPT. It exposes a
...
WORKGPT, it means a deeper local runtime rather than a thin model
...
```bash
kun serve \
  --config ~/.workgpt/kun/config.json \
  --host 127.0.0.1 \
  --port 8899 \
...
```bash
kun run --data-dir ~/.workgpt/kun --workspace "$PWD" "summarize this repo"
kun chat --data-dir ~/.workgpt/kun --workspace "$PWD"
kun exec --data-dir ~/.workgpt/kun --workspace "$PWD" --list-tools
kun exec --data-dir ~/.workgpt/kun --workspace "$PWD" read --args '{"path":"README.md"}'
```
...
After the legacy provider retirement, the WORKGPT main process
