  ...
  it('accepts GUI plan context on start turn payloads', () => {
    const parsed = StartTurnRequest.parse({
      ...
      guiPlan: {
        ...
        relativePath: '.workgpt/plan/auth.md',
        planId: '/tmp/ws:.workgpt/plan/auth.md',
        sourceRequest: 'Add auth',
        title: 'Auth'
      }
    })
    expect(parsed.guiPlan?.relativePath).toBe('.workgpt/plan/auth.md')
  ...
  it('rejects unsafe GUI plan context paths on start turn payloads', () => {
    const result = StartTurnRequest.safeParse({
      ...
      guiPlan: {
        operation: 'draft',
        workspaceRoot: '/tmp/ws',
        relativePath: '.workgpt/plan/nested/auth.md',
        planId: 'plan_bad'
      }
    })
    expect(result.success).toBe(false)
...
describe('cli', () => {
  ...
  it('reads OpenAI-compatible API keys from OPENAI_API_KEY when no DeepSeek key is set', () => {
    const parsed = parseServeOptions(['--data-dir', '/tmp/kun'], { OPENAI_API_KEY: 'sk-openai' })

    expect(parsed.apiKey).toBe('sk-openai')
  })

  it('prefers DEEPSEEK_API_KEY over OPENAI_API_KEY for compatibility', () => {
    const parsed = parseServeOptions(['--data-dir', '/tmp/kun'], {
      DEEPSEEK_API_KEY: 'sk-deepseek',
      OPENAI_API_KEY: 'sk-openai'
    })

    expect(parsed.apiKey).toBe('sk-deepseek')
  })
