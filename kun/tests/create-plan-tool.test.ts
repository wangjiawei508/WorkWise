    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# legacy',
        operation: 'draft',
        plan_relative_path: '.workgpt/plan/legacy.md'
      },
      buildContext({ threadMode: 'plan', workspace: '/tmp/ws' })
    )
    expect(result.isError).toBe(true)
...
describe('create_plan tool: success and atomic write', () => {
  ...
  beforeEach(async () => {
    ...
    await mkdir(join(workspace, '.workgpt/plan'), { recursive: true })
    await writeFile(join(workspace, '.workgpt/plan/login.md'), previousMarkdown, 'utf8')
    ...
    const result = await executeCreatePlanTool(
      ...
      buildContext({
        ...
        guiPlan: {
          operation: 'draft',
          workspaceRoot: workspace,
          relativePath: '.workgpt/plan/login.md',
          planId: `${workspace}:.workgpt/plan/login.md`
        }
      })
    )
    expect(result.isError).toBe(true)
  ...
  it('overwrites an existing plan when the same reserved path is reused', async () => {
    ...
    expect(result.isError).toBeFalsy()
    const persisted = await readFile(join(workspace, '.workgpt/plan/login.md'), 'utf8')
