...
维护者：[wangjiawei508](https://github.com/wangjiawei508)。WORKGPT 保留原项目致谢，并在此基础上继续补充桌面发布、写作导出、插件市场、连接手机和工程监测 Skills。
...
- **Markdown 导出**：PDF 使用内置 Chromium 离屏渲染并等待字体/图片加载；DOC 以 Word 兼容 HTML 保存；DOCX 使用随桌面包携带的平台转换器，并在不可用时回退到 WORKGPT 内置 DOCX 生成器。
