...
最新源码还提供独立的广义 w / VCE / Huber / 统计家族高级模型试算：显式声明完整模型与依据，保存原始输入、逐轮结果及不可变历史，恢复和 JSON 导出前严格重算。试算保留不可检测、不可辨识、负方差和未收敛状态，不自动删除观测或更新正式权。Huber 使用显式固定外部尺度，统计家族提供正态/t/卡方尾概率与完整家族 Bonferroni 校正，缺失成员仍计入分母。详见[输入与记录合同](./docs/qa/RAILWISE_SURVEY_ADVANCED_TRIALS_BACKEND.md)；新界面的精确包验收另行记录。
