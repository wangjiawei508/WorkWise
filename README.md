...
最新源码还提供独立的广义 w / VCE 高级模型试算：显式声明完整模型与依据，保存原始输入、逐轮结果及不可变历史，恢复和 JSON 导出前严格重算。试算保留不可检测、不可辨识、负方差和未收敛状态，不自动删除观测或更新正式权。详见[输入与记录合同](./docs/qa/RAILWISE_SURVEY_ADVANCED_TRIALS_BACKEND.md)；新界面的精确包验收另行记录。
