<p align="center">
  <img src="src/asset/img/workgpt.png" width="96" alt="WORKGPT 图标">
</p>
...
[项目主页](https://github.com/wangjiawei508/WORKGPT) | [下载](https://github.com/wangjiawei508/WORKGPT/releases)

[![GitHub release](https://img.shields.io/github/v/release/wangjiawei508/WORKGPT?label=github)](https://github.com/wangjiawei508/WORKGPT/releases)
[![License](https://img.shields.io/github/license/wangjiawei508/WORKGPT)](./LICENSE)

> Fork / attribution: this project is a renamed fork/import of `AdrianAndroid/DeepSeek-GUI`. See [FORK_NOTICE.md](./FORK_NOTICE.md).

WORKGPT 是一个面向开发者和高频 AI 工作者的本地桌面工作台。它以 Kun 为唯一运行时，把终端里的智能体体验整理成更容易上手、更适合长期使用的应用：选择工作目录，发起任务，实时查看推理、工具调用和文件改动，并在需要时审批或回退。
...
<p align="center">
  <a href="src/asset/img/code.mp4">
    <img src="src/asset/img/code.gif" width="410" alt="WORKGPT Code 模式演示">
  </a>
  <a href="src/asset/img/write.mp4">
    <img src="src/asset/img/write.gif" width="410" alt="WORKGPT 写作模式演示">
  </a>
</p>
...
- **Write 写作模式**：独立管理 `~/.workgpt/write_workspace` 和自定义写作空间，读取 Markdown 文件树，支持 Live / Source / Split / Preview 四种视图、相对图片预览、DeepSeek FIM 短补全 / 灵感长补全（可用跨文本 BM25 + 关键词检索增强）、当前文档导出为 `HTML / PDF / DOC / DOCX`，以及选中文本后直接唤起 inline 写作助手。
- **Markdown 导出**：PDF 使用内置 Chromium 离屏渲染并等待字体/图片加载；DOC 以 Word 兼容 HTML 保存；DOCX 会优先调用随桌面包携带或环境变量配置的 pandoc，其次尝试平台 `md2docx`，最后使用 WORKGPT 内置 DOCX 生成器兜底。
- **帮助与更新**：设置页内置帮助中心，提供 GitHub 主页、下载页、README 和问题反馈入口；GUI 更新会优先使用配置的 R2/generic feed，没有 R2 时自动回落到 GitHub Releases 在线检查。
...
- **跨平台使用**：提供 macOS `.dmg/.zip`（Apple Silicon 与 Intel）和 Windows x64 `.exe`；也可以从源码构建。

## 运行时：Kun

WORKGPT 当前唯一活跃的本地 Agent 运行时是仓库自带的
...
Renderer (React)
  → KunRuntimeProvider
  → preload: workgpt.runtimeRequest / startSse
...
WORKGPT 现在以 **Code** 和 **写作** 两个主工作台为核心，并提供
...
<p align="center">
  <img src="src/asset/img/codemode.png" alt="WORKGPT Code 模式" width="860">
</p>
...
<p align="center">
  <img src="src/asset/img/writemode.png" alt="WORKGPT Write 模式" width="860">
</p>

- 管理 `~/.workgpt/write_workspace` 与多个自定义写作空间，左侧文件树支持新建、重命名与删除。
...
| 导出 DOCX | 优先使用随应用打包的 `pandoc` 或 `WORKGPT_PANDOC_PATH` / `PANDOC_PATH` 指定的 pandoc；pandoc 不可用时尝试平台 `md2docx`；仍不可用时使用 WORKGPT 内置 DOCX 生成器，覆盖标题、段落、列表、引用、代码块、表格、链接和本地图片。 |
...
| `md2docx_macos.zip` | macOS 平台转换器 | 可通过 `npm run prepare:converters` 提取到 `converters/darwin-arm64/`，打包 Apple Silicon 版本时随应用携带；其中 `pandoc` 作为高保真 DOCX 后端，`md2docx.bin` 作为可发现的备用工具。 |
| `md2docx_win.zip` / `md2docx_winV2.zip` | Windows x64 平台转换器 | 可通过 `npm run prepare:converters` 提取到 `converters/win32-x64/`，Windows x64 安装包会随应用携带 `pandoc.exe` / `md2docx.exe`。 |
...
目前你提供的 macOS 转换器包是 Apple Silicon 版本；Intel Mac 包仍会生成，也能使用 WORKGPT 内置 DOCX 生成器，但如果要让 Intel 包也内置 pandoc，需要再把 Intel 版 `pandoc` 放到 `converters/darwin-x64/pandoc`。

也可以在开发机上用环境变量指定外部 pandoc：

```bash
# macOS / Linux
export WORKGPT_PANDOC_PATH=/opt/homebrew/bin/pandoc

# Windows PowerShell
$env:WORKGPT_PANDOC_PATH="C:\\Program Files\\Pandoc\\pandoc.exe"
```
...
<p align="center">
  <img src="src/asset/img/clawmode.png" alt="WORKGPT 连接手机" width="860">
</p>
...
前往 [GitHub Releases](https://github.com/wangjiawei508/WORKGPT/releases) 下载最新版本：
...
Write 模式的目标是把 WORKGPT 从“代码/聊天工作台”扩展成真正可长期写作的桌面工作区。实现时参考了本地 `openhanako` 项目中的几个方案：
...
- 打开“设置 -> 应用 -> 已安装的应用”，找到 `WORKGPT` 并卸载。
- 或在“控制面板 -> 程序和功能”中卸载。
- 也可以运行安装目录中的卸载程序。

Windows 安装器默认会创建开始菜单和桌面快捷方式。安装包不会强制固定到任务栏；如需固定，可在开始菜单中右键 `WORKGPT` 并选择固定。

### macOS

- 将 `WORKGPT.app` 从“应用程序”移到废纸篓。
- 如果首次打开被系统拦截，可在 Finder 中右键应用并选择“打开”。
- 本地未公证构建可先运行：

```bash
npm run mac:unquarantine -- '/Applications/WORKGPT.app'
```
...
| 平台 | 应用数据位置 |
| --- | --- |
| macOS | `~/Library/Application Support/WORKGPT` |
| Windows | `%APPDATA%\WORKGPT` |

Kun 数据默认位于 `~/.workgpt/kun` 或应用数据目录下的 Kun data dir。删除前请确认其中没有你还需要的会话、MCP 或 Skill 配置。

---

## 更新

WORKGPT 支持在线检查 GUI 新版本：

- 普通用户：在 **设置 → 通用 → GUI 更新** 点击“检查更新”，应用会优先读取已配置的 R2/generic 更新源；如果没有配置 R2，会自动回落到 [GitHub Releases](https://github.com/wangjiawei508/WORKGPT/releases)。
- macOS：只有签名并公证的正式构建才能一键下载并重启安装；未签名或本地开发构建会显示 GitHub 下载页，手动下载安装包覆盖即可。
- Windows：正式 x64 安装器支持应用内下载；如遇网络或权限问题，可直接从 Releases 下载最新 `.exe` 安装。
- 发布者：Release workflow 会上传 macOS arm64/x64、Windows x64 安装包和 `latest*.yml` 到 GitHub Release；如果配置了 R2 secrets，还会同步发布 R2 latest feed。

可选环境变量：

| 变量 | 说明 |
| --- | --- |
| `WORKGPT_GITHUB_REPO` | 指定在线检查使用的 GitHub 仓库，默认 `wangjiawei508/WORKGPT`。 |
| `WORKGPT_UPDATE_URL` / `WORKGPT_UPDATE_URL_STABLE` / `WORKGPT_UPDATE_URL_FRONTIER` | 指定 electron-updater generic feed；支持 `{channel}` 占位符。 |
| `R2_PUBLIC_BASE_URL` / `R2_RELEASE_PREFIX` | 发布到 R2 时生成 generic feed 路径。 |
| `WORKGPT_DOWNLOAD_URL` | 覆盖设置页“打开下载页”的目标地址。 |
| `WORKGPT_ALLOW_UNSIGNED_UPDATES=1` | 仅用于本地调试，允许未签名 macOS 构建尝试自动更新。 |
...
- 所有为 WORKGPT 提交 issue、建议、代码和文档的贡献者。

<a href="https://github.com/wangjiawei508/WORKGPT/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=wangjiawei508/WORKGPT" />
</a>
...
[![Star History Chart](https://api.star-history.com/chart?repos=wangjiawei508/WORKGPT&type=date&legend=top-left)](https://www.star-history.com/?repos=wangjiawei508%2FWORKGPT&type=date&logscale=&legend=top-left)
