...
- 用户手动做一次性短语替换时，会先用确定性规则把同段其他同短语一起替换，例如 `workgpt -> WORKGPT`。
...
此外，术语大小写和简单重命名不完全依赖模型。编辑器会对一次性短语替换做同段传播：当你把 `workgpt` 改成 `WORKGPT` 或 `DXGUI`，同一个自然段里其他 `workgpt` 会同步替换。这个确定性层负责“必须发生”的一致性，recent edits 和 RAG 再负责后续 AI 编辑时理解这种意图。
...
- `src/main/ipc/app-ipc-schemas.ts`、`src/preload/index.ts`、`src/shared/workgpt-api.ts`：复用 `write:inline-completion` IPC，通过 `mode: "edit"`、`editCandidate` 和 recent edits 承载 inline edit。
