# WorkWise 0.3.6 候选准备与历史验收记录（更新于 2026-08-11）

## 结论

- 当前正式 stable 为 `v0.3.5`，下一个待验收补丁候选已确定为 `0.3.6`。
- 已安装并截图验收的签名产物来自已废弃的内部 `0.4.1` 候选提交 `66562c8`；只保留为更新机制和界面回归的历史证据，不得当作 `0.3.6` 产物验收。
- 未创建 `v0.3.6` 标签或 GitHub Release，未将 `0.3.6` 推广到 stable，未将它更新到官网正式下载入口。
- 2026-08-11 已经用户确认执行 stable 回滚：R2、官网 stable 指针和 GitHub Latest 均已恢复为 `v0.3.5`，三份公开清单均已独立验证为 `0.3.5`。
- `v0.4.0` 标签、Release 和安装资产保留供审计；Release 已标记“已撤回”和 prerelease，不再是 Latest。由于没有实际下载用户，不再为它开发降级或升级桥接。
- GitHub Actions 曾在 macOS arm64、macOS x64、Windows x64 上完成真实 `0.3.5 -> 私有 0.4.1` 应用内更新闭环；正式验收 `0.3.6` 前必须重做 `0.3.5 -> 私有 0.3.6` 闭环。
...
- 已废弃的内部 `0.4.1` 签名候选已完成 1171x768 浅色中文界面截图检查，未发现文本或按钮重叠。
- 深色主题与窄窗口有同一功能代码基线的历史截图记录；最终 `0.3.6` 安装包必须重新复核。

已废弃的内部 `0.4.1` 签名候选截图证据（仅用于回归对比）：
...
- 历史内部 `0.4.1` 候选 CI Run：`31437775903`。
...
- 上述结果证明更新机制可用，但不代表 `0.3.6` 安装包已验收。

## 自动化与产物校验

- 当前 `0.3.6` 版本路线变更已在本机通过 TypeScript、ESLint、生产构建、247 个主应用测试文件（1781 项测试）和 75 个 Runtime 测试文件（641 项测试）。
- 版本路线切换前的功能分支提交 `2f63604` 已通过 GitHub Actions Quality 门禁：Run `31450396008`。切换为 `0.3.6` 后仍需在新提交上重跑 CI。
- stable 回滚工作流通过：Run `31450414453`；工作流使用精确确认口令，未重新构建或发布 Release。
- 历史内部 `0.4.1` 候选的生产打包检查已通过；`0.3.6` 尚未生成候选安装包。
- 历史 macOS arm64 候选 ZIP：`WorkWise-0.4.1-mac-arm64.zip`（不是 `0.3.6` 待发布产物）。
...
1. 尚未基于最终 `0.3.6` 提交生成并安装三平台签名候选产物，也未重做 `0.3.5 -> 私有 0.3.6` 应用内更新闭环。
2. 更新清单服务器仍返回七天缓存响应头；需要服务器管理员或控制面板权限修复，当前部署账户无权安全修改 Nginx/OpenResty 配置。
3. `0.3.6` 的 macOS x64 与 Windows x64 仍需重做原生更新闭环，并在发布前确认是否接受以自动化安装证据替代两平台人工截图。
4. 用户尚未对最终 `0.3.6` 待发布安装包的界面、功能和具体发布动作给出最终批准。
