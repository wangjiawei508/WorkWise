...
- 截至 2026-08-11，公开 GitHub Release 和 stable 三份清单仍指向错误的 `0.4.0`；候选代码中的撤回状态尚未改变公开分发状态。
...
1. 公开 stable 仍指向错误的 `0.4.0`。`v0.3.5` 三平台正式安装包均存在，可作为回滚来源，但变更 stable 或 GitHub Release 状态必须先获得用户对准确动作的明确批准。
2. 用户尚未确认本轮截图、功能和准确正式版本号，也未明确批准创建标签、GitHub Release、推广 stable 或更新官网。
3. 更新清单服务器仍返回七天缓存响应头；需要服务器管理员或控制面板权限修复，当前部署账户无权安全修改 Nginx/OpenResty 配置。
4. macOS x64 与 Windows x64 已完成原生更新闭环，但发布前仍需确认是否接受以自动化安装证据替代两平台人工截图。
5. 候选分支需在发布前同步最新 `origin/main`，重新执行最终门禁并确认最终产物与本轮已验收应用代码一致。
