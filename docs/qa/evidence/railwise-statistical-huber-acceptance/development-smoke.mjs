...
try {
  for (const name of ['huber-unique', 'huber-flat', 'statistical-reversed']) {
    const declarationJson = await readFile(join(evidence, 'inputs', `${name}.json`), 'utf8')
    const request = { kind: name.startsWith('huber') ? 'huber' : 'statistical-family', acknowledged: true,
