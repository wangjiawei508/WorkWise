...
scoring oracle. Requires Node for exact ECMAScript JSON serialization of manifest
doubles; imports no product modules. No services, HTTP or write SQL are called. Run while
...
from scoring_oracle import require, sha, load_json, verify_row
...
def ecmascript_json(payload):
    result = subprocess.run(['node', str(HERE / 'ecmascript-json.mjs')], input=payload,
                            text=True, capture_output=True, timeout=15)
    require(result.returncode == 0, 'ECMAScript JSON serialization failed')
    return result.stdout


def json_text(value, sort=False):
    # Engineering manifests contain doubles. Python's exponent/decimal formatting
    # differs from JSON.stringify; use the standard ECMAScript serializer only.
    return ecmascript_json(json.dumps({'value': value, 'sort': sort}, ensure_ascii=True,
