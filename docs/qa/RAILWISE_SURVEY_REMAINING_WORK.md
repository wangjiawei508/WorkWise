...
当前最终候选状态：`in_progress`。签名构建、安装及实机结果由主任务独立留证，本清单不预判通过。98 项中 80 项勾选、18 项未勾选；这是历史增量任务计数，**不是总计划完成率**，也不是18个互不重叠的功能缺陷。
...
| 任务号 / 行 | 剩余范围 | 分类与证据 |
| --- | --- | --- |
| 15 / 23，完整 Engineering 界面验收 | CSV/XLSX 新时间算法、完整历史趋势和中文报告须在最终包重做三格式/清单/重启；线程隔离、空/错/过期状态、主题尺寸及键盘仍须全矩阵证据。7d4f454 的 XLSX/图表失败保留。 | 包验收。[监测修复归档](evidence/railwise-monitoring-correction-20260920/README.md)。 |
| 29 / 46，完整验证集合 | 源码测试、类型、构建和 strict 已有通过记录；余项为最终精确包 GUI 与绑定证据，不是缺测试设施。 | 包验收。[执行台账最新节](RAILWISE_SURVEY_CONVERGENCE_STATUS.md)。 |
...
| 43 / 68，精确包总门禁 | 签名公证、隔离安装和真实私有 updater 已多次完成；最终包仍需自己的身份、安装、两 P0 链及重启证据。真实旧用户数据覆盖、用户本人和专业确认仍缺。 | 包验收＋外部＋人审。[私有更新流程](PRIVATE_SURVEY_UPDATER_ACCEPTANCE.md)。 |
...
3. **监测严格重算。** 新 `workwise-engineering-2` 修正时间排序与图表一致性，并保留旧记录；[verifyDeliverable](../../kun/src/engineering/engineering-service.ts) 的 `inputs` 只检查绑定快照，`surveyReplay` 只重算 Survey adjustment/deformation。纯监测清单仍为 `surveyReplay=not-applicable`，缺按记录算法重算全部监测 results 的独立检查。新分析算法回归不等于此项补齐。
...
先完成已冻结最终包的监测/P0/恢复实操并绑定精确工件；再把任务39、44、45、46等软件缺口拆成独立实现与验收边界，外业/点云/长尾遵循 P0 门禁。真实材料与人审先备齐可审成果和明确待确认项，不以模拟专家意见关闭生产或签认门禁。
