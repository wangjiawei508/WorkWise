# WorkWise Agent Runtime 缓存优化技术文档

本文记录 WorkWise Agent Runtime 运行时当前的缓存优化设计、实现位置、
...
WorkWise Agent Runtime 借鉴了 Reasonix 的 cache-first 设计，但按 GUI 场景做了收束：
...
WorkWise Agent Runtime 使用独立的稳定系统提示词文件：

- `kun/src/prompt/kun-system-prompt.ts`

这个前缀只承载长期稳定的运行契约，例如：

- WorkWise Agent Runtime 身份
- GUI 调用边界
- 工具行为约束
- 缓存行为约束
- 回复风格与质量要求
...
WorkWise Agent Runtime 通过 `ImmutablePrefix` 管理系统 prompt、tools、pinned constraints
...
工具集合本身就是 prompt prefix 的一部分。WorkWise Agent Runtime 在发送请求前会统一做：
...
WorkWise Agent Runtime 当前在模型请求边界对消息做一层共享的 model history repair：
...
WorkWise Agent Runtime 也会在模型请求边界做一层 Reasonix 风格的 history hygiene：
...
- WorkWise Agent Runtime 会写入一个 error `tool_result`，让模型收敛到更窄的查询或解释原因。
...
WorkWise Agent Runtime 的缓存命中统计优先使用 DeepSeek 原生 usage 字段：
...
WorkWise Agent Runtime 也会把单轮真实 `prompt_tokens` 作为下一次请求的 compaction pressure。
...
WorkWise Agent Runtime 已覆盖这些关键行为：
...
GUI 通过 WorkWise Agent Runtime 使用统一接口读取 usage：
...
2026-06-02 的 WorkWise Agent Runtime 实测结果：
...
这个文档强调一个产品级约束：缓存优化属于 WorkWise Agent Runtime，而不是 GUI。
...
- 架构总览：`docs/runtime-architecture.md`
- WorkWise Agent Runtime 贡献指南：`docs/runtime-contributing.md`
- WorkWise Agent Runtime 使用说明：`kun/README.md`
