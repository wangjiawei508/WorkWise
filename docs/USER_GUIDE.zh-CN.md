...
| 候选版验收中 | Flow Preview；Design 结构化画板、Agent 改图、PPTX 导入导出、PNG/SVG 导出和插入 Write |
...
原“定时任务”会在首次启动时幂等迁移为“定时触发器 → Agent”Flow，并保留启用状态、模型、提示词、工作区、下次运行时间和历史线程引用。运行历史可查看每个节点的输入、输出和错误，也可审批、取消、从失败节点重试或从可恢复检查点继续。
