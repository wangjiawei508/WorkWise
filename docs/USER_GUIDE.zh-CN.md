...
| 候选版验收中 | Design 结构化画板、Agent 改图、PPTX 导入导出、PNG/SVG 导出和插入 Write |
| 可选能力 | MinerU 高精度解析、在线 Skill 更新、连接手机、定时任务和外部 CLI |
| 路线图 | Flow、新增多模态模型和更多行业智能体 |
...
- Design 助手可以通过当前画板命令添加、修改、删除、组合和取消组合元素。它只操作当前工作区、当前文档和当前页面。
- 可直接导出 PNG、SVG 和经过验证的 PPTX。
- 可把当前页面作为 PNG/SVG 插入 Write 文档。
- 导入 PPTX 时会尽量保留页面顺序、图片、层级、旋转、group 和可编辑路径；无法无损映射的效果会显示保真警告。
...
PPT Master 使用固定上游提交的受控瘦身快照，不是完整官方仓库。它保留核心 `SKILL.md`、100 个文本参考、工作流、55 个顶层脚本及模块、76 个图表模板、5 个品牌、2 个 Deck、20 页通用布局、确认页和两个轻量示例；不包含大体积图标 SVG、AI 对比 PNG、用户真实 projects、生成 exports、backup 或私有 PPT 文件。运行脚本需要 Python 3.10+，本地确认页和实时预览需要 `flask`；WorkWise 不安装 Pandoc。
