# WorkWise Agent Runtime cache optimization technical documentation

This article records the cache optimization design, implementation location, and implementation of the current WorkWise Agent Runtime runtime.
...
WorkWise Agent Runtime's cache optimization serves four goals:
...
- Let the GUI only bear the responsibility of HTTP/SSE calls, and consolidate the caching discipline inside WorkWise Agent Runtime.
...
WorkWise Agent Runtime therefore treats token optimization as a combined strategy, not a
...
WorkWise Agent Runtime borrowed Reasonix’s cache-first design, but adapted it to the GUI scenario:
...
WorkWise Agent Runtime manages system prompts, tools, and pinned constraints through `ImmutablePrefix`
...
The toolset itself is part of the prompt prefix. WorkWise Agent Runtime will do the following before sending a request:
...
WorkWise Agent Runtime currently performs a shared model history repair on messages at the model request boundary:
...
WorkWise Agent Runtime will also do a layer of Reasonix-style history hygiene at model request boundaries:
...
- WorkWise Agent Runtime will write an error `tool_result` to let the model converge to a narrower query or explain the reason.
...
WorkWise Agent Runtime's cache hit statistics preferentially use DeepSeek's native usage field:
...
WorkWise Agent Runtime will also use a single round of real `prompt_tokens` as the compaction pressure for the next request.
...
WorkWise Agent Runtime has covered these key behaviors:
...
The GUI uses a unified interface to read usage through WorkWise Agent Runtime:
...
WorkWise Agent Runtime actual measurement results on 2026-06-02:
...
This document emphasizes a production-level constraint: cache optimization belongs to WorkWise Agent Runtime, not the GUI.
...
- WorkWise Agent Runtime Contributing Guide: `docs/runtime-contributing.md`
- WorkWise Agent Runtime usage instructions: `kun/README.md`
