# WorkWise Agent Runtime 贡献指南:架构、设计模式与代码组织思想

> 适用对象:任何想向 `kun/` 包贡献代码的工程师。
> 阅读路径:第一次贡献可以按顺序读完全文;之后再回来查找具体章节。
> 配套文档:[`runtime-architecture.md`](./runtime-architecture.md)
...
**WorkWise Agent Runtime 是一个严格执行 Ports & Adapters + Functional Core / Imperative
...
WorkWise Agent Runtime 的目录本身就是六边形的物理布局:
...
WorkWise Agent Runtime 同时使用了多个经典模式,它们互相加强而不是冲突。
...
- `docs/runtime-architecture.md` 描述"为什么这样设计";
- `kun/README.md` 描述 CLI / env / data dir;
- `docs/runtime-contributing.md`(本文)描述"如何贡献";
...
- [ ] 不修改任何 `docs/runtime-architecture.md` 描述以外的文件
      来"绕过" typecheck
...
- [ ] 新增 SSE 事件时,给 `docs/runtime-architecture.md` 加一行
- [ ] 在 `docs/runtime-architecture.md` 的 borrow map 中(如未来
...
> **WorkWise Agent Runtime 是一个严格执行 Ports & Adapters + Functional Core /
