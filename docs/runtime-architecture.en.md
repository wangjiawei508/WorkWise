# WorkWise Agent Runtime GUI single-runtime architecture

This document describes how the WorkWise Agent Runtime desktop app should now be organized around one dedicated runtime,
`WorkWise Agent Runtime`, that serves the GUI through a single HTTP/SSE boundary.
...
Renderer (React + Zustand)
  Code / Write / Connect phone UI
        |
        | window.workwise.runtimeRequest(path, method, body)
        | window.workwise.startSse(threadId, sinceSeq)
        v
Preload IPC bridge
        |
        v
...
WorkWise Agent Runtime cache-hit metrics should be computed and optimized using DeepSeek native fields first:
...
- `kun/src/prompt/kun-system-prompt.ts` is the stable prefix.
  It may only contain long-lived WorkWise Agent Runtime run contract content and must not include
...
- Tool-collection mutation policy: adding tools should be append-only; edit/reorder/remove
  ...
  Current WorkWise Agent Runtime canonicalizes schema, but this mutation policy still needs explicit product-level
  enforcement.
...
Renderer should only expose WorkWise Agent Runtime. The UI sections listed below should be removed or
kept removed:
...
- Remove diagnostic/importer modules unrelated to WorkWise Agent Runtime.

Main process now only needs:

- `kunRuntimeAdapter`: start/stop `kun serve`, sync config, calculate base URL,
and append auth headers.
- `runtimeRequestViaHost`: forward `/v1/*` after ensuring WorkWise Agent Runtime is running.
- `startSse` / `stopSse`: forward WorkWise Agent Runtime SSE streams keyed by `threadId + sinceSeq`.
...
WorkWise Agent Runtime GUI HTTP must expose the same capabilities previously exposed through CodeWhale:
...
- `POST /v1/sessions/{id}/resume-thread` follows the previous CodeWhale resume path.
  WorkWise Agent Runtime should first attempt same-name thread restore, then session snapshot/JSONL reconstruction,
...
- `GET /v1/usage?group_by=thread|day` returns accumulated token/turn/cache-hit counters.
  Workbench home and composer footer consume WorkWise Agent Runtime usage only and do not open runtime
  insight panels.
...
WorkWise Agent Runtime packages are organized by ports & adapters:
...
and forwards results. When adding capability, add WorkWise Agent Runtime tool or HTTP endpoint first,
...
1. Open the WorkWise Agent Runtime desktop app.
...
4. Connect phone can save settings, run manual tasks, and write thread IDs back to WorkWise Agent Runtime mapping.
5. `Settings -> Agents` shows only WorkWise Agent Runtime, with no provider switch and no runtime diagnostics/
...
7. Thread search, archive, fork/resume, and request_user_input answer/cancel flows all operate
   through WorkWise Agent Runtime HTTP paths.
