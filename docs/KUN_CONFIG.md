...
本文说明 WORKGPT / Kun 的本地配置文件在哪里、哪些字段由 UI 管理、哪些字段适合手工扩展，以及模型上下文压缩阈值应该如何配置。
...
1. GUI settings
   ...
   - macOS: `~/Library/Application Support/WORKGPT/workgpt-settings.json`
   - Windows: `%APPDATA%/WORKGPT/workgpt-settings.json`
   - Linux: `~/.config/WORKGPT/workgpt-settings.json`
...
1. GUI 读取 `workgpt-settings.json`，得到 `agents.kun` 和通用 provider 配置。
