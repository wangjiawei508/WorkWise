...
window:
  ...
  app_icon: src/asset/img/workgpt.png
  secondary_logos: [workgpt.svg]
...
WORKGPT is a local desktop workbench for the **Kun**
...
Every screen in WORKGPT follows the same macro-grammar:
...
- The product name is "WORKGPT". The runtime is "Kun".
...
│       │                                                      │
│       │ window.workgpt.runtimeRequest / startSse              │
│       ▼                                                      │
...
- **Preload** (`src/preload/`) — `contextBridge` surface.
  Exposes a typed `window.workgpt` API to the renderer. No Node
...
src/
  ...
  preload/
    index.ts                        # contextBridge surface (window.workgpt)
...
### 6.3 The workgpt API surface

`window.workgpt` is the only thing the renderer is allowed to call
...
Every method on this surface is typed in `src/shared/workgpt-api.ts`
...
| Threads / turns / events | `~/.workgpt/kun` | JSON + JSONL | Kun |
...
`WORKGPT_STARTUP_TRACE=1` and prints to stdout for
postmortem timing.
...
- **Renderer isolation** — `contextIsolation: true`, no
  ...
  only sees the `window.workgpt` API surface.
...
| workgpt API | `src/preload/index.ts`, `src/shared/workgpt-api.ts` |
